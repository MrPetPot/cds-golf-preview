...
if (creditosImagenes) {
  ...
  const promocionesCred = PROMOS.map(p => {
    ...
    if (p.foto) return `<li><strong>${p.name}</strong><span>${credito(p.foto) || 'Archive image'}</span></li>`;
  ...
  creditosImagenes.innerHTML = `
    <details class="image-credits">
      ...
      <div class="image-credit-groups">
        ...
        <section><h3>Golf courses</h3><ul>${camposCred.map(f => `<li><strong>${(CX[f.id] && CX[f.id].name) || f.id}</strong><span>${[credito(f), f.uso].filter(Boolean).join(' · ')}</span></li>`).join('')}</ul></section>
      </div>
    </details>`;
}
...
(function () {
  ...
  const TX = {
    pestana: 'Follow the ranking',
    cerrar: 'Close',
    kicker: 'PRIMEandGOLF · EDITION ',
    titulo: 'Get the next edition',
    ...
    opPro: 'I work in development, sales or property investment.',
    opOtro: 'Another interest.',
    rango: 'Indicative range',
    rango1: 'Up to €1M',
    rango2: '€1–3M',
    rango3: '€3–5M',
    rango4: 'Over €5M',
    ...
    hor2: 'Next 12 months',
    hor3: 'Next 6 months',
    avisoPromo: 'You may write to me if you come across a development that is especially relevant to my search.',
    empresa: 'Company',
    opcional: 'optional',
    actividad: 'Activity',
    act1: 'Developer',
    act2: 'Sales agent or agency',
    act3: 'Investor or fund',
    act4: 'Architecture or consultancy',
    act5: 'Other',
    guardar: 'Save',
